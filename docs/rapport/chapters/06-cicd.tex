...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lllX}
        ...
        cd-frontend.yml & Push sur main & CD & Build Angular, sync S3, invalidation CF \\
...
\begin{enumerate}
    ...
    \item Invalidation du cache CloudFront
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Push vers Amazon ECR (tag: SHA du commit + latest)
    \item Mise a jour de la Task Definition ECS
    \item Deploiement du service ECS avec attente de stabilite
\end{enumerate}
...
Les credentials AWS Learner Lab expirent toutes les \~{}4 heures.
Un script \texttt{scripts/update-aws-keys.sh} permet de mettre a jour
simultanement~:
\begin{itemize}
    \item \texttt{\~{}/.aws/credentials} (CLI locale)
    \item Les GitHub Secrets (via \texttt{gh secret set})
\end{itemize}
...
% TODO: Captures d'ecran GitHub Actions
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{github-actions-ci.png}
